...
Upserts capture the control row under the existing document claim, perform the normal fenced write/completion, then strong-read the control row again. A missing or changed epoch prevents a readiness acknowledgement. Ambiguous control-row writes retain the claim's uncertainty window. Successful upserts return `index_incarnation` alongside stored processing versions.
