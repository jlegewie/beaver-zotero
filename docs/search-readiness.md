...
The plugin background runtime owns the summary. Its existing reconciler publishes an inventory only after successful enumeration and processing-state reconciliation, fenced against account, scope and preference invalidation. Identified additions and deletions update a complete inventory immediately: additions count as pending and deletions leave the counts. Unresolved notifications use a separate membership fence until targeted replay, without rejecting the in-progress inventory. Notifications during rediscovery retain this fence so enumeration cannot overwrite a newer membership change. Unsupported formats, linked URLs, trash, trashed parents and excluded libraries are omitted. A failed or partial inventory is not complete. Missing ledger rows count as pending.
