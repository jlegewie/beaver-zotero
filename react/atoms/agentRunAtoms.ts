 ...
 *
 * Returns the ids of the actions actually reverted. An action type with no
 * undo branch is deliberately NOT in that list; see the `else` at the bottom
    ...
    for (const { action } of indexed) {
        try {
            if (isCreateAnnotationsAgentAction(action)) {
                ...
                // `undoneActionIds` is the accurate outcome — they stay 'applied'
                // instead of being flipped to 'undone' by an undo that never ran.
                //
