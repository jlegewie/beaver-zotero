...
export async function getFileStatusForAttachmentInfo(attachmentItem: Zotero.Item): Promise<BeaverStatusInfo> {
    ...
    try {
        ...
        const fileStatus = planFeatures.advancedProcessing ? attachmentStatus.advanced_status : attachmentStatus.md_status;
        const errorCode = planFeatures.advancedProcessing ? attachmentStatus.advanced_error_code : attachmentStatus.md_error_code;
        if (!fileStatus) {
            return { text: 'Unknown status', showButton: false };
        }
