...
/**
 * Extract a "result count" for search-like tool results.
 *
 * Notes:
 * - Returns `null` if the tool return doesn't represent a countable result set.
 * - Returns `0` when the tool is recognized but has no results.
 * - For fulltext search tools, counts unique attachments (matches what we render).
 */
export function extractToolResultCount(part: ToolReturnPart): number | null {
    const { tool_name, content, metadata } = part;

    // Item search tools -> items count
    if (isItemSearchResult(tool_name, content, metadata)) {
        return extractItemSearchData(content, metadata)?.items.length ?? 0;
    }
    ...
    // External search tools -> references count
    if (isExternalSearchResult(tool_name, content, metadata)) {
        const references = extractExternalSearchData(content, metadata)?.references;
