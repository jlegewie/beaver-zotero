...
import { CitationData, CitationMetadata, getUniqueKey } from '../../types/citations';
...
export const AgentRunFooter: React.FC<AgentRunFooterProps> = ({ run }) => {
    ...
    const getToolDetails = (part: ToolCallPart): string => {
        ...
        const count = result?.content?.attachments?.length || result?.content?.references?.length || 0;
        
        let details = `[${label}`;
        if (query) details += `: "${query}"`;
        if (result) details += ` (${count} results)`;
        details += `]`;
        return details;
    };
