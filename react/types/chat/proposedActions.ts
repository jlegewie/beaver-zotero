...
/**
 * Extract proposed actions from metadata
 */
export function actionsFromMetadata(metadata: unknown): ProposedAction[] {
    if (!metadata) return [];

    const rawActions = Array.isArray(metadata)
        ? normalizeRawActions(metadata)
        : normalizeRawActions(
              (metadata as any)?.proposed_actions ??
                  (metadata as any)?.actions
          );
    ...
    return rawActions
        ...
        .filter(
            (action): action is ProposedAction => action !== null
        );
}

/**
 * Merge existing and incoming proposed actions
 */
export function mergeActions(
    existing: ProposedAction[] | undefined,
    ...
    const byId = new Map(
        incoming.map((action) => [action.id, action] as const)
    );
    const merged = existing.map((action) => {
        ...
        return {
            ...
            result_data: update.result_data ?? action.result_data,
            error_message:
                update.error_message ?? action.error_message ?? undefined,
        };
    });
