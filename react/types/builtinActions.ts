...
export const BUILTIN_ACTIONS: Action[] = [
...

    {
        ...
        text: 'Find papers closely related to this item.\n\nSteps:\n1. Identify what makes this paper distinctive: its topic, method, data or population, and central claim. The abstract and metadata are usually enough; for a long document without a useful abstract, use the extract tool if available instead of reading it.\n2. Run external searches from more than one of those angles so the results are not one-dimensional. If external search is not available, say so in one line and tell me I can switch to the Beaver model or enable Plus Tools in settings, then search my own library from those same angles instead. Never present library results as if they were new work found outside it.\n3. Check which results are already in my library.\n\nFormat: a ranked list of up to 8 papers. For each, give authors (year), title, and one sentence on why it is relevant. Clearly mark the papers I already have, and cite the rest as external references so I can add them. If you fell back to searching my library, say so at the top and mark every result as one I already have.\n\nIf the item has no abstract or full text, work from the title and metadata and say so. If my message names a specific kind of similarity (same method, same data, competing argument), prioritize it. If multiple items are attached, handle each separately.',
...

    {
        ...
        text: 'Assign tags to the attached items.\n\nSteps:\n1. Look at my existing tags first and reuse them wherever they fit, matching my style and level of granularity. If my message states a naming convention (casing, separators, a prefix, a fixed vocabulary), every tag you apply must follow it exactly.\n2. Work from each item\'s abstract and metadata and assign 2-5 subject tags covering the topic and, where clear, the method. Many items have no abstract: tag those from the title where the topic is clear, and open the attachment only for a handful of genuinely ambiguous ones — do not try to read full text across a large batch.\n3. Be consistent: items on the same subtopic must share the same tags, but decide per item — if one tag set ends up covering most of the items, you are generalizing instead of tagging. Create a new tag only when nothing existing fits, and keep new tags lowercase and 1-3 words.\n\nFor more than about 50 items, run this as one batch job over all of them; below that, tag them directly.\n\nWhen done, show a table of items and their assigned tags, marking newly created tags so I can spot vocabulary drift; for a large set, give the counts and a representative sample instead of every item. Cover every attached item, and if any were tagged from the title alone or left untagged, list them explicitly instead of presenting the job as complete.',
        targets: ['items', 'collection'],
        category: 'organize',
        sortOrder: 140,
    },

    {
        ...
        text: 'File the attached items into my existing collections.\n\nSteps:\n1. Review my collections (including sub-collections) to understand how my library is organized.\n2. Add each item to the collection or collections where it clearly belongs — the best one or two, not every plausible match. Never remove an item from a collection it is already in.\n3. If an item fits no existing collection, do not force it: list it separately and suggest a sensible new collection name for it.\n\nFor more than about 50 items, run this as one batch job over all of them; below that, file them directly.\n\nWhen done, show a table of items and where each was filed, with the no-fit items and suggested new collections at the end; for a large set, give the counts per destination and a representative sample instead of every item.',
        targets: ['items', 'collection'],
        category: 'organize',
        sortOrder: 150,
    },

    // Revived under its original id (previously archived) so users who kept
    // running `/fix-metadata` keep their command, overrides, and history.
    {
        id: 'builtin-fix-metadata-recent',
        name: 'fix-metadata',
        title: 'Fix metadata',
        description: 'Fill in missing abstracts, DOIs, and other core fields, verified against external sources for a set of items or everything in a collection.',
        text: 'Fix missing or incomplete metadata for the attached items or everything in a collection.\n\nSteps:\n1. Check each item\'s metadata with `get_metadata`: the abstract, DOI, authors, year, and publication or publisher. Focus on what is genuinely missing or clearly wrong (a title in ALL CAPS, a noun where an author should be). Skip fields that are legitimately absent for the item type, such as a DOI for a book.\n2. Get correct values from `lookup_work` (by DOI, ISBN, or exact title) rather than guessing. If nothing reliable turns up, read only the first page or two of the attachment; do not extract whole documents for metadata. Leave a field empty rather than filling it with a guess.\n3. Apply the fixes with `edit_metadata`, one item at a time, changing only the fields that were missing or wrong.\n\nFor more than about 30 items, run this as one batch job over all of them; below that, fix them directly. Most items in a screening pass need nothing; that is expected.\n\nWhen done, report what you changed grouped by item, and list the items where a field is still missing because you could not verify a value; for a large set, give the counts per field and a representative sample instead of every item.',
        argumentHint: 'Optional: what to fix, e.g. abstracts or DOIs',
        targets: ['items', 'collection'],
        category: 'organize',
        sortOrder: 160,
    },
...

    {
        ...
        text: 'Organize the items in this collection into thematic sub-collections.\n\nSteps:\n1. Review existing sub-collections and whether they contain any items. Reuse existing sub-collections whenever they fit.\n2. Design a scheme of roughly 3-7 sub-collections with clear, mutually distinct themes and reasonably balanced sizes. Prefer themes that reflect how the material would actually be used, not surface keywords. If my message specifies an organizing principle (by method, by period, by debate), use it.\n3. Lay out the proposed scheme in your message first — each sub-collection with an item count and 2-3 example items. State it as your recommendation and keep going; do not stop to ask me to confirm it, since I approve the changes themselves as you call the tools. Ask me a question only if the scheme turns on a choice you genuinely cannot make for me (e.g. which organizing principle to use), and then ask that one question with the concrete alternatives as options.\n4. Create the new sub-collections first, then add each item to the one where it belongs — for more than about 40 items directly in this collection, as one batch job over them (not those already in a sub-collection). Items that fit nowhere stay where they are — list them at the end rather than forcing them in.\n\nFinish with a summary of the final structure.',
...

    {
        ...
        text: 'Create highlight annotations based on my request.\n\nSteps:\n1. Determine the scope:\n   - If I have a document open or attached, work within that document.\n   - If I have a collection selected, or my request names one, the scope is the documents in that collection.\n   - If my request names a topic across my library (for example, "all definitions of social capital"), search my library to find the relevant documents first. When the target is clear — a document is open or attached, a collection is selected, or my request names specific passages or a specific topic — proceed. Ask me to confirm the scope first only when it is genuinely ambiguous, such as a library-wide request with no clear target.\n2. Locate the matching passages and create a highlight on the 1-3 sentences that actually match, each with a short comment explaining why it matters. Use in-attachment search or the extract tool (if available) rather than reading each document end to end. Be selective — a few precise highlights beat many loose ones. If the scope is more than about 15 documents, run it as one batch job across them; a document with no matching passage is a normal outcome, not a failure.\n3. Write a structured summary of what you highlighted (see format below).\n\nFormat the summary as a clear, scannable overview, not a long write-up:\n- Open with one sentence stating the scope and how many highlights you made across how many documents.\n- Then one section per document, each with a bold heading (the document title). Under each heading, give 2-4 sentences in prose describing what you highlighted there and why it matters, citing the new annotations inline. Keep it tight; do not restate every highlight verbatim. Documents with no match get one line each, not a section.\n- If only one document is involved, skip the per-document headings and give a single short section.\n- Close with a **Recap** that ties the highlights together. When multiple documents are involved, this recap must compare across them: where they agree, differ, or complement each other, and what the overall picture is. When only one document is involved, use the recap to state the single most important takeaway.\n\nKeep the whole summary readable at a glance: bold headings, short prose, no walls of text.',
...

    {
        ...
        text: "Help me tidy up my library. Work in four steps.\n\nStep 1 — Explore. Find what actually needs attention without changing anything yet. Look at my recent additions and, using library search, my unfiled items (items in no collection). Assess a few kinds of mess: items not filed into any collection, items with no or too few tags, paper-like items missing an abstract or DOI, whether I use tags and collections at all, and any unusually large collection that has outgrown itself. Note the `total_count` of each search — that is the size of the job, and Step 3 works all of it — so do not page through the results here.\n\nTips to find messes:\n- Load the `zotero_library_organization`, `zotero_advanced_search` and `batch_jobs` capabilities together in your first step. If you expect to fix metadata in Step 3, load `zotero_metadata_editing` in that same step rather than discovering it mid-task.\n- Understand collection and tag structure: `list_collections` and `list_tags` for the main user library. Note whether the user uses tags or collections to organize their library.\n- Find recent items, unfiled items and items without tags:\n    - Recent items: `list_items` for the main user library with 'sort_by': 'dateAdded', 'sort_order': 'desc', 'limit': 20, 'item_category': 'regular'\n    - Unfiled items: `zotero_search` with condition `{'field': 'unfiled', 'operator': 'true'}`\n    - Not tagged: `zotero_search` with condition `{'field': 'tag', 'operator': 'doesNotContain', 'value': ''}`\n    - Use `get_metadata` to check recent items for incorrect or missing metadata (no abstract, missing DOI for journal articles etc)\n\nStep 2 — Choose what to do first. If my message already said what to focus on, use that. Otherwise decide for yourself: weigh how I actually organize my library (tags vs collections, and how I use them) against the kinds of mess you found in Step 1, pick the 1-2 highest-impact tasks, and go straight to Step 3 — don't stop to ask. Open your Step 3 summary with one line naming what you're starting with and noting that I can redirect you. A batch asks me to approve it before anything changes, showing the item count and where the work lands, so that approval is my chance to redirect you. Only pause to ask me first — with the `ask_user_question` tool, or plain text if it is unavailable — when the messes are genuinely different in kind and you truly cannot judge which I would want; if you do ask, keep it to the 2-3 highest-priority tasks, phrase each with a specific count, and include an 'All of it' option. Prefer starting the work over asking: an unanswered question stalls the run for two minutes and helps no one. Examples of tasks worth prioritizing:\n- Sort 100s of unfiled items into collections\n- Assign tags to 100s of untagged items\n- Clean up metadata for 100+ items missing an abstract or DOI\n- If I barely use tags: Design a small, consistent tag vocabulary for my library\n- If I have a large collection: Split collection 'PhD Thesis' into topic-based sub-collections\n- Clean up 112 tags (remove duplicates, merge similar tags, ensure consistent naming) — not a batch job: it works on the tag list with `manage_tags`, so keep it to a reviewable chunk of about 20 tags and say what remains\n\nStep 3 — Do the work. Take one task at a time, over its whole population. When that population is more than about 35 items, run it as one batch job; at or below that, just do the work directly and report it — a batch's approval card and progress tracking are overhead a job this size does not need. Whenever possible, make decisions based on metadata alone without reading the fulltext of attachments.\n   - Collections: add each item to the existing collection(s) where it clearly belongs; propose a new collection only when a cluster of items shares a clear theme that fits nothing existing. Never remove an item from a collection it is already in.\n   - Tags: assign 1-4 subject tags per item, reusing my existing tags wherever they fit and keeping any new tags lowercase and 1-3 words.\n   - Metadata: fill only genuinely missing fields (especially the abstract and DOI) verifying against external sources rather than guessing. Skip fields that are legitimately absent for an item type (for example, a DOI for a book). Most items in a screening pass need nothing — that is expected.\n   Finish with a compact summary of what changed: the counts plus a short sample of the items so I can spot-check. If a population was larger than one batch could hold, say how many remain.\n\nStep 4 — Suggest a next step. Based on what you saw, offer one useful follow-up and wait for me to confirm before doing it. Examples of useful follow-up work:\n- Propose the next task from Step 2 if more remain, or the remainder of a population that was too large for one batch\n- Propose to look for invalid or missing data beyond recently added items. For example:\n    - Missing an abstract: `zotero_search` with condition `{'field': 'abstractNote', 'operator': 'doesNotContain', 'value': ''}` — add `'fields': ['DOI']` to also see each item's DOI inline and spot items missing that too, without a separate `get_metadata` call\n    - Missing a DOI: `zotero_search` with condition `{'field': 'DOI', 'operator': 'doesNotContain', 'value': ''}` (note: books, reports and similar types legitimately have no DOI, so treat this count as approximate)\n",
