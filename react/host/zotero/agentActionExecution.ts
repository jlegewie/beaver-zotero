...
const APPLY_EXECUTORS = new Map<string, (action: AgentAction, runId: string) => Promise<any>>([
    ...
    ['edit_note', (action) => executeEditNoteVariantAction(action)],
    ['edit_note_batch', (action) => executeEditNoteVariantAction(action)],
    ['edit_note_blocks', (action) => executeEditNoteVariantAction(action)],
...
const UNDO_EXECUTORS = new Map<string, (action: AgentAction) => Promise<void>>([
    ['edit_note', undoEditNoteVariantAction],
    ['edit_note_batch', undoEditNoteVariantAction],
    ['edit_note_blocks', undoEditNoteVariantAction],
...
async function applyClaimedActions(
    ...
    try {
        if (isAnyEditNoteActionType(actionType)) {
...
async function undoClaimedActions(set: Setter, actions: AgentAction[]): Promise<UndoAgentActionsResult> {
    ...
    try {
        ...
        const errorMessage = isAnyEditNoteActionType(actionType)
...
export const rejectAgentActionsAtom = atom(
    ...
    (get, set, { actions }: { actions: AgentAction[] }): void => {
        ...
        for (const action of actions) {
            ...
            if (isAnyEditNoteActionType(action.action_type)) {
