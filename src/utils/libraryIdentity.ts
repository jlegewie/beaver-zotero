/**
 * Zotero-side resolution for Beaver's device-portable library identity.
 *
 * The `library_ref` grammar itself (parsing, the `"u"` / `"g<groupID>"`
 * format, and its distinction from the search-index scope ref in
 * `zoteroUtils.ts`) lives in `libraryRef.ts` and is re-exported here in full,
 * so either module serves as an import site for it. This module adds the
 * Zotero-dependent half: resolving a `library_ref` (or legacy `library_id`)
 * against this device's local `Zotero.Libraries` / `Zotero.Groups`, and
 * resolving item references to live `Zotero.Item` objects.
 *
 ...
 */
import {
    EXTERNAL_FILE_LIBRARY_SENTINEL,
    LIBRARY_REF_PATTERN,
    parseItemReference,
    parseLibraryRef,
