 ...
 *
 * The "call read_note again" clause is load-bearing, not filler: a token minted
 * by an older Beaver build lands here too (the format is versioned), and a
 * thread resumed across that upgrade has no current token to copy. Without the
 * clause the only instruction is to copy something that does not exist.
 */
const SNAPSHOT_MALFORMED_MESSAGE =
    ...
    + 'do not shorten, edit or reconstruct it. If you do not have one, call read_note again and use '
...
async function validateEditNoteBlocksAction(
    ...
    // ── Numeric addressing ──────────────────────────────────────────────────
    // Structural garbage first: a mismatch verdict cannot tell a fabricated
...
export function planBlockEditsExecution(inputs: BlockExecutionInputs): BlockExecutionPlan {
    ...
    // validator). `checkSnapshotStatus` is sync — the no-await contract holds.
    const snapshotStatus = checkSnapshotStatus(snapshot, noteId, simplified);
    if (!soleRewrite && snapshotStatus === 'malformed') {
        return { ok: false, error: SNAPSHOT_MALFORMED_MESSAGE, errorCode: 'snapshot_malformed' };
    }
    if (!soleRewrite && snapshotStatus === 'mismatch') {
