...
import { modelObjectIdFromReference, resolveObjectId, UNRESOLVED_LIBRARY_ID } from '../../../utils/libraryIdentity';
...
async function createAnnotations(args: any, highlight: boolean): Promise<any> {
    const operation = captureInstanceOperation();
    try {
        ...
        const response = await handleAgentActionExecuteRequest({
            ...
            action_data: { ...data, ...validation.normalized_action_data }, operation,
        });
