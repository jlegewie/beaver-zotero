...
/** MCP adapters and registration, independent of chat renderers. */
...
import { getCitationKeyFromItem, getZoteroSelectURI } from '../../utils/zoteroUtils';
import {
...
const PORT = argv[2] || env.BEAVER_MCP_PORT || "23119";
...
export async function handleCreateNote(args: any): Promise<any> {
    ...
    const response: WSAgentActionExecuteResponse = await handleAgentActionExecuteRequest({
        ...executeRequest, timeout_seconds: 120, operation,
    });
...
export function registerMcpServer(writeToolsEnabled: boolean) {
        logger('McpServer: MCP server enabled, registering endpoint', 3);

        // Write the stdio bridge script for MCP clients like Claude Desktop
        ensureMcpBridgeScript().catch((err) => {
            logger(`McpServer: Failed to write MCP bridge script: ${err?.message}`, 1);
        });

        const service = new MCPService();
        service.setAuthCheck(() => !!Zotero.Beaver.account?.getSnapshot().session);
        ...
        for (const { def, handler } of tools) {
            service.registerTool(def.name, def, async args => {
                ...
                if (!active || !account?.getSnapshot().session) return mcpError('MCP access unavailable');
                const result = await handler(args);
                if (!active || generation !== account.getGeneration() || scope !== JSON.stringify(Zotero.Beaver.searchableLibraryIds)) return mcpError('MCP access changed');
                return result;
            });
        }
        ...
        return () => {
            active = false;
            if (registered) {
                logger('McpServer: Cleaning up MCP endpoint', 3);
                service.unregister();
            }
        };
}
