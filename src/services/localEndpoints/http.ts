...
} from './handlers/testExclusionHandlers';
import {
...
} from './handlers/testPdfHandlers';
import { captureInstanceOperation } from './operation';
import { createEndpoint, registerEndpoint } from './registration';
import { handleArtifactRequest } from '../artifacts/artifactProvider';
/** Local HTTP routes, owned by the plugin instance. */
...
} from './handlers/testAnnotationHandlers';
...
} from './handlers/testBackgroundHandlers';
...
} from './handlers/testCollectionHandlers';

import {
    handleTestEpubExtractHttpRequest,
} from './handlers/testEpubHandlers';

import {
    handleTestLibraryIdentityHttpRequest,
} from './handlers/testLibraryIdentityHandlers';
...
async function handleAgentActionValidateHttpRequest(request: any) {
    ...
    const response = await handleAgentActionValidateRequest({ ...wsRequest, operation: captureInstanceOperation() });
...
async function handleAgentActionExecuteHttpRequest(request: any) {
    ...
    const response = await handleAgentActionExecuteRequest({ ...wsRequest, operation: captureInstanceOperation() });
...
async function handleTestProviderConnectHttpRequest(request: any) {
    await Zotero.Beaver.localEndpoints!.provider.connect({
        wakeId: request?.wake_id,
        wakeInstanceId: request?.wake_instance_id,
    });
    return { ok: true, status: Zotero.Beaver.localEndpoints!.provider.getStatus() };
}

async function handleTestProviderStatusHttpRequest(_request: any) {
    return Zotero.Beaver.localEndpoints!.provider.getStatus();
}

async function handleTestProviderCloseHttpRequest(_request: any) {
    Zotero.Beaver.localEndpoints!.provider.close();
    return { ok: true, status: Zotero.Beaver.localEndpoints!.provider.getStatus() };
}
...
export function registerEndpoints(): (() => void) | undefined {
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        endpoints['/beaver/test/batch-progress-preview'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/batch-progress-preview', request));

        endpoints['/beaver/test/batch-progress-clear'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/batch-progress-clear', request));
        ...
        endpoints['/beaver/test/note-create'] = createEndpoint(handleTestNoteCreateHttpRequest);

        endpoints['/beaver/test/note-delete'] = createEndpoint(handleTestNoteDeleteHttpRequest);

        endpoints['/beaver/test/note-read'] = createEndpoint(handleTestNoteReadHttpRequest);

        endpoints['/beaver/test/note-open-editor'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/note-open-editor', request));

        endpoints['/beaver/test/note-close-editor'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/note-close-editor', request));

        endpoints['/beaver/test/note-undo'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/note-undo', request));

        endpoints['/beaver/test/note-apply'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/note-apply', request));

        endpoints['/beaver/test/note-preview'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/note-preview', request));
        ...
        endpoints['/beaver/test/pdf-retrieval'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/pdf-retrieval', request));

        endpoints['/beaver/test/pdf-captcha-eligibility'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/pdf-captcha-eligibility', request));
        ...
        endpoints['/beaver/test/pdf-page-count'] = createEndpoint(handleTestPdfPageCountHttpRequest);

        endpoints['/beaver/test/pdf-page-labels'] = createEndpoint(handleTestPdfPageLabelsHttpRequest);

        endpoints['/beaver/test/pdf-render-pages'] = createEndpoint(handleTestPdfRenderPagesHttpRequest);

        endpoints['/beaver/test/pdf-render-pages-with-meta'] = createEndpoint(handleTestPdfRenderPagesWithMetaHttpRequest);

        endpoints['/beaver/test/pdf-extract-raw-detailed'] = createEndpoint(handleTestPdfExtractRawDetailedHttpRequest);

        // orchestration parity endpoints
        endpoints['/beaver/test/pdf-extract'] = createEndpoint(handleTestPdfExtractHttpRequest);

        endpoints['/beaver/test/pdf-extract-paragraph'] = createEndpoint(handleTestPdfExtractParagraphHttpRequest);

        endpoints['/beaver/test/pdf-has-text-layer'] = createEndpoint(handleTestPdfHasTextLayerHttpRequest);

        endpoints['/beaver/test/pdf-analyze-ocr'] = createEndpoint(handleTestPdfAnalyzeOcrHttpRequest);

        endpoints['/beaver/test/pdf-search-scored'] = createEndpoint(handleTestPdfSearchScoredHttpRequest);

        endpoints['/beaver/test/pdf-sentence-bboxes'] = createEndpoint(handleTestPdfSentenceBBoxesHttpRequest);
        ...
        endpoints['/beaver/test/pdf-render-overlay'] = createEndpoint(handleTestPdfRenderOverlayHttpRequest);
        ...
        endpoints['/beaver/test/pdf-extract-trace'] = createEndpoint(handleTestPdfExtractTraceHttpRequest);
        ...
        // debugging.
        endpoints['/beaver/test/pdf-analyze-layout'] = createEndpoint(handleTestPdfAnalyzeLayoutHttpRequest);
        ...
        endpoints['/beaver/test/epub-annotation-parity'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/epub-annotation-parity', request));

        // Snapshot annotation selector/sortIndex parity.
        endpoints['/beaver/test/snapshot-annotation-parity'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/snapshot-annotation-parity', request));

        // Reader position (`getCurrentPage` / `content_kind`) and the EPUB
        // citation-navigation path against the live reader.
        endpoints['/beaver/test/reader-state'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/reader-state', request));

        endpoints['/beaver/test/epub-citation-navigate'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/epub-citation-navigate', request));

        // Citation host: itemData.resolveItemDisplay (icon item type +
        // readable-attachment availability for cited-source rows).
        endpoints['/beaver/test/resolve-item-display'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/resolve-item-display', request));
        ...
        endpoints['/beaver/test/processing-ledger'] = createEndpoint(handleTestProcessingLedgerHttpRequest);

        endpoints['/beaver/test/processing-reset'] = createEndpoint(handleTestProcessingResetHttpRequest);
        ...
        endpoints['/beaver/test/excluded-libraries'] = createEndpoint(handleTestExcludedLibrariesHttpRequest, { allowScopeChange: true });

        // Exclusion-gated handlers that have no production HTTP route.
        endpoints['/beaver/test/get-annotations'] = createEndpoint(handleTestGetAnnotationsHttpRequest);

        endpoints['/beaver/test/view-images'] = createEndpoint(handleTestViewImagesHttpRequest);

        endpoints['/beaver/test/attachment-image'] = createEndpoint(handleTestAttachmentImageHttpRequest);
        ...
        endpoints['/beaver/test/new-thread'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/new-thread', request));
        endpoints['/beaver/test/chat-send'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/chat-send', request));
        endpoints['/beaver/test/current-ids'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/current-ids', request));
        endpoints['/beaver/test/load-thread'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/load-thread', request));
        endpoints['/beaver/test/list-actions'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/list-actions', request));
        endpoints['/beaver/test/approve-action'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/approve-action', request));
        endpoints['/beaver/test/confirm-credits'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/confirm-credits', request));
        endpoints['/beaver/test/undo-action'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/undo-action', request));

        endpoints['/beaver/test/application-state'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/application-state', request));

        endpoints['/beaver/test/beaver-window'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/beaver-window', request));

        endpoints['/beaver/test/open-table'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/open-table', request));

        endpoints['/beaver/test/close-table'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/close-table', request));

        // `openTable` itself — the product path the item-pane button takes.
        endpoints['/beaver/test/open-stored-table'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/open-stored-table', request));
        ...
        endpoints['/beaver/test/table-create'] = createEndpoint(handleTestTableCreateHttpRequest);

        endpoints['/beaver/test/table-read'] = createEndpoint(handleTestTableReadHttpRequest);

        endpoints['/beaver/test/table-list'] = createEndpoint(handleTestTableListHttpRequest);
        ...
        endpoints['/beaver/test/table-write'] = createEndpoint(handleTestTableWriteHttpRequest);

        endpoints['/beaver/test/table-edit'] = createEndpoint(handleTestTableEditHttpRequest);

        endpoints['/beaver/test/table-versions'] = createEndpoint(handleTestTableVersionsHttpRequest);

        endpoints['/beaver/test/table-revert'] = createEndpoint(handleTestTableRevertHttpRequest);

        endpoints['/beaver/test/table-trim'] = createEndpoint(handleTestTableTrimHttpRequest);

        endpoints['/beaver/test/table-delete'] = createEndpoint(handleTestTableDeleteHttpRequest);

        endpoints['/beaver/test/table-open'] = createEndpoint(handleTestTableOpenHttpRequest);

        endpoints['/beaver/test/table-corrupt'] = createEndpoint(handleTestTableCorruptHttpRequest);
        ...
        endpoints['/beaver/test/table-shadow'] = createEndpoint(handleTestTableShadowHttpRequest);

        endpoints['/beaver/test/table-restore-shadow'] = createEndpoint(handleTestTableRestoreShadowHttpRequest);
        ...
        endpoints['/beaver/test/table-open-reader'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/table-open-reader', request));

        endpoints['/beaver/test/table-view-state'] = createEndpoint(handleTestTableViewStateHttpRequest);

        // The item-pane section (dev-only): whether it is registered, and the
        // fields it would render for one table.
        endpoints['/beaver/test/table-item-pane'] = createEndpoint(handleTestTableItemPaneHttpRequest);

        endpoints['/beaver/test/beaver-sidebar'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/beaver-sidebar', request));

        endpoints['/beaver/test/select-tab'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/select-tab', request));

        endpoints['/beaver/test/run-status-popup'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/run-status-popup', request));

        endpoints['/beaver/test/quick-prompt'] = createEndpoint(request => Zotero.Beaver.runtime.dispatchWindowCommand('/beaver/test/quick-prompt', request));
