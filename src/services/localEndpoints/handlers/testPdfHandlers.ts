...
} from '../../../beaver-extract/debug/overlayBuilders';
import type { OverlayResult } from '../../../beaver-extract/debug/overlayBuilders';
import { drawBBoxOverlayPNG } from '../canvasOverlay';
import type {
    BoundingBox,
} from '../../../beaver-extract';
import { projectAnalyzeLayout } from '../../../beaver-extract/debug/analyzeLayoutProjection';
import { projectTracePage } from '../../../beaver-extract/debug/traceProjection';
...
export async function handleTestPdfExtractHttpRequest(request: any) {
    const { BeaverExtractor } = await import('../../../beaver-extract');
...
export async function handleTestPdfSentenceBBoxesHttpRequest(request: any) {
    const { getMuPDFWorkerClient } = await import('../../../beaver-extract');
...
export async function handleTestPdfRenderOverlayHttpRequest(request: any) {
    ...
    if (!language && request?.library_id != null && request?.zotero_key != null) {
        try {
            const { getItemLanguage } = await import('../../../utils/zoteroUtils');
