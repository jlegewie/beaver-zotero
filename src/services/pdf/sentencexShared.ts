/**
 * Worker-safe sentencex helpers — pure byte/language normalization with
 * zero ChromeUtils, Zotero, or `applyPostProcessing` dependency.
 *
 * Imported by both the main-thread `SentencexSplitter.ts` (which adds the
 * ChromeUtils-based loader on top) and the worker-side splitter resolver
 * (`worker/splitterResolver.ts`). Keeping these pure here lets the worker
 * bundle reuse the byte/char conversion + language normalization logic
 * without dragging the chrome-context loader into worker scope.
 *
 * Imports limited to `./SentenceMapper` types so the file stays portable
 * for vitest (no chrome:// resolution).
 */
