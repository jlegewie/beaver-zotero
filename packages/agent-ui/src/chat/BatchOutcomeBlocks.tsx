 ...
 */
function joinRows(block: BatchOutcomeBlock, items?: BatchItemsRecord, collectionLibraryRef?: string): OutcomeEntry[] {
    const rows = block.rows ?? [];
    const entries: OutcomeEntry[] = rows.map((row) => ({
        row,
        group: batchItemGroupFor(items, block, row, collectionLibraryRef),
    }));
...
function countRows(batch: BatchProgressEntry, items?: BatchItemsRecord): number {
    return (batch.blocks ?? []).reduce((sum, block) => sum + joinRows(block, items, batch.operation === 'sort' ? batch.library_ref : undefined).length, 0);
}
...
export const BatchOutcomeBlockView: React.FC<{
    ...
    const joined = joinRows(block, items, operation === 'sort' ? libraryRef : undefined);
...
const BatchFilteredBlocks: React.FC<{
    ...
    const nothingMatches =
        ...
        && !(batch.blocks ?? []).some((block) =>
            joinRows(block, items, batch.operation === 'sort' ? batch.library_ref : undefined).some((entry) => matchesFilter(entry, filter, population)),
        );
    return (
        <>
