...
// =============================================================================
// Literal-dollar retry (math expansion round-trip asymmetry)
// =============================================================================

describe('literal-dollar retry', () => {
    // Emulates production expandToRawHtml just enough for the retry seam:
    // math-wraps $…$ pairs by default and shields individually selected
    // literal dollars during matcher retries.
    const mathAwareExpand = (str: string, ...rest: unknown[]) => {
        const options = rest[5] as {
            expandMath?: boolean;
            literalDollarRanges?: Array<{ start: number; end: number }>;
            literalDollarOrdinals?: number[];
        } | undefined;
        if (options?.expandMath === false) return str;
        const protectedDollars: string[] = [];
        const unescapedDollarPositions = new Set<number>();
        for (let i = 0; i < str.length; i++) {
            if (str.charAt(i) !== '$') continue;
            let backslashes = 0;
            for (let j = i - 1; j >= 0 && str.charAt(j) === '\\'; j--) backslashes++;
            if (backslashes % 2 === 0) unescapedDollarPositions.add(i);
        }
        let dollarOrdinal = 0;
        const shielded = str.split('').map((char, index) => {
            const isUnescapedDollar = unescapedDollarPositions.has(index);
            const preserve = char === '$' && (
                !isUnescapedDollar
                || options?.literalDollarOrdinals?.includes(dollarOrdinal)
                || options?.literalDollarRanges?.some(
                    (range) => index >= range.start && index < range.end,
                )
            );
            if (isUnescapedDollar) dollarOrdinal++;
            if (preserve) {
                const placeholder = `__LITERAL_DOLLAR_${protectedDollars.length}__`;
                protectedDollars.push(char);
                return placeholder;
            }
            return char;
        }).join('');
        return shielded.replace(
            /(?<!\$)\$(?!\$)(?=\S)((?:[^$\\]|\\.)+?)(?<=\S)\$(?!\$)/g,
            (m) => `<span class="math">${m}</span>`,
        ).replace(
            /__LITERAL_DOLLAR_(\d+)__/g,
            (_match, index) => protectedDollars[Number(index)],
        );
    };

    it('matches note-literal dollar text via the retry with math expansion disabled', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            // The note stores the dollar pair VERBATIM (no math elements), so
            // the math-wrapped needle can never occur in the raw HTML.
            const result = match(makeInput({
                oldString: 'the value of $x+y$ in the model',
                newString: 'the value of $x+z$ in the model',
                strippedHtml: '<p>Context here. the value of $x+y$ in the model. Tail.</p>',
            }));
            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe('the value of $x+y$ in the model');
            // new_string keeps the note's literal-dollar convention too.
            expect(result?.expandedNew).toBe('the value of $x+z$ in the model');
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    });

    it('expands independent inserted math while preserving the literal-dollar anchor', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'JSON keys "$schema" and "$ref"';
            const injected = ' Formula: $x$';

            const validationResult = match(makeInput({
                operation: 'insert_after' as EditNoteOperation,
                oldString,
                newString: injected,
                strippedHtml: `<p>${oldString}</p>`,
            }));
            expect(validationResult?.strategy).toBe('exact');
            expect(validationResult?.expandedOld).toBe(oldString);
            expect(validationResult?.expandedNew).toBe(
                ' Formula: <span class="math">$x$</span>',
            );

            const executionResult = match(makeInput({
                operation: 'insert_after' as EditNoteOperation,
                oldString,
                newString: oldString + injected,
                strippedHtml: `<p>${oldString}</p>`,
            }));
            expect(executionResult?.expandedNew).toBe(
                `${oldString} Formula: <span class="math">$x$</span>`,
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('preserves genuine math in a target that also has literal dollars', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'Formula $x$ uses "$schema" and "$ref"';
            const newString = 'Formula $y$ uses "$schema" and "$ref"';
            const result = match(makeInput({
                oldString,
                newString,
                strippedHtml: '<p>Formula <span class="math">$x$</span> uses "$schema" and "$ref"</p>',
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe(
                'Formula <span class="math">$x$</span> uses "$schema" and "$ref"',
            );
            expect(result?.expandedNew).toBe(
                'Formula <span class="math">$y$</span> uses "$schema" and "$ref"',
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('does not preserve an LCS-aligned dollar without retained anchor context', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'JSON keys "$schema" and "$ref"';
            const result = match(makeInput({
    ...
    it('keeps rewritten currency delimiters literal without shielding independent math', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput({
                oldString: 'US$5/CA$7',
                newString: 'EUR$6;GBP$8',
                strippedHtml: '<p>Prices: US$5/CA$7.</p>',
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe('US$5/CA$7');
            expect(result?.expandedNew).toBe('EUR$6;GBP$8');
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    });

    it('does not shield the opening delimiter of complete numeric math', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput({
                oldString: 'US$5/CA$7',
    ...
    it('preserves a compact literal pair when only its body changes', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput({
                oldString: '$a$',
            ...
            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe('$a$');
            expect(result?.expandedNew).toBe('$b$');
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    });

    it('shields rewritten currency before pairing adjacent replacement math', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput({
                oldString: 'US$5/CA$7',
                newString: 'Cost EUR$6;formula:$x$',
                strippedHtml: '<p>US$5/CA$7</p>',
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedNew).toBe(
                'Cost EUR$6;formula:<span class="math">$x$</span>',
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('does not count escaped math dollars as delimiter ordinals', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'Formula $x+\\$5$ uses "$schema" and "$ref"';
            const newString = 'Formula $y+\\$6$ uses "$schema" and "$ref"';
            const result = match(makeInput({
                oldString,
                newString,
                strippedHtml: '<p>Formula <span class="math">$x+\\$5$</span> uses "$schema" and "$ref"</p>',
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe(
                'Formula <span class="math">$x+\\$5$</span> uses "$schema" and "$ref"',
            );
            expect(result?.expandedNew).toBe(
                'Formula <span class="math">$y+\\$6$</span> uses "$schema" and "$ref"',
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('keeps escaped literal dollars out of math while expanding adjacent genuine math', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'Cost \\$5;$x$';
            const newString = 'Cost \\$6;$y$';
            const result = match(makeInput({
                oldString,
                newString,
                strippedHtml: '<p>Cost \\$5;<span class="math">$x$</span></p>',
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe(
                'Cost \\$5;<span class="math">$x$</span>',
            );
            expect(result?.expandedNew).toBe(
                'Cost \\$6;<span class="math">$y$</span>',
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('keeps adjacent numeric math after a lowercase label as math', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const oldString = 'JSON "$schema" and "$ref"';
            const result = match(makeInput({
                oldString,
                newString: 'JSON "$schema" and "$ref";formula$2+2$',
                strippedHtml: `<p>${oldString}</p>`,
            }));

            expect(result?.strategy).toBe('exact');
            expect(result?.expandedNew).toBe(
                'JSON "$schema" and "$ref";formula<span class="math">$2+2$</span>',
            );
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it('applies literal-dollar retry to the single-edit Markdown-render fallback', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const renderedOld = '<pre><code>{"$schema":"schema.json","$ref":"defs.json"}</code></pre>';
            const renderedNew = '<pre><code>{"$schema":"schema.json","$ref":"updated.json"}</code></pre>';
            const result = findMarkdownRenderMatch(makeInput({
                oldString: '```json\n{"$schema":"schema.json","$ref":"defs.json"}\n```',
                newString: '```json\n{"$schema":"schema.json","$ref":"updated.json"}\n```',
                strippedHtml: `<div>${renderedOld}</div>`,
                renderedOldSimplified: renderedOld,
                renderedNewSimplified: renderedNew,
            }));

            expect(result?.strategy).toBe('markdown_render');
            expect(result?.expandedOld).toBe(renderedOld);
            expect(result?.expandedNew).toBe(renderedNew);
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    ...
    it.each([
        {
            ...
            oldString: 'the value of $x+y$ in the model\n',
            newString: 'the value of $x+z$ in the model\n',
            strippedHtml: '<p>the value of $x+y$ in the model</p>',
            expectedOld: 'the value of $x+y$ in the model',
            expectedNew: 'the value of $x+z$ in the model',
        },
        {
            strategy: 'json_unescape',
            oldString: 'the value of $x+y$\\nnext line',
            newString: 'the value of $x+z$\\nnext line',
            strippedHtml: '<p>the value of $x+y$\nnext line</p>',
            expectedOld: 'the value of $x+y$\nnext line',
            expectedNew: 'the value of $x+z$\nnext line',
        },
        {
            strategy: 'spurious_wrap_strip',
            oldString: '<p>the value of $x+y$ in the model</p>',
            newString: '<p>the value of $x+z$ in the model</p>',
            strippedHtml: '<div>the value of $x+y$ in the model</div>',
            expectedOld: 'the value of $x+y$ in the model',
            expectedNew: 'the value of $x+z$ in the model',
        },
        {
            strategy: 'tag_attribute_strip',
            oldString: '<p style="font-weight: bold">the value of $x+y$</p>',
            newString: '<p>the value of $x+z$</p>',
            strippedHtml: '<p>the value of $x+y$</p>',
            expectedOld: '<p>the value of $x+y$</p>',
            expectedNew: '<p>the value of $x+z$</p>',
        },
    ])('preserves literal-dollar mode through $strategy re-expansion', (testCase) => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput(testCase));
            expect(result?.strategy).toBe(testCase.strategy);
            expect(result?.expandedOld).toBe(testCase.expectedOld);
            expect(result?.expandedNew).toBe(testCase.expectedNew);
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    });

    it('still matches real math elements on the first pass (retry never fires)', () => {
        vi.mocked(expandToRawHtml).mockImplementation(mathAwareExpand as any);
        try {
            const result = match(makeInput({
                oldString: '$x+y$',
                newString: '$x+z$',
                strippedHtml: '<p>Formula: <span class="math">$x+y$</span> holds.</p>',
            }));
            expect(result?.strategy).toBe('exact');
            expect(result?.expandedOld).toBe('<span class="math">$x+y$</span>');
        } finally {
            vi.mocked(expandToRawHtml).mockImplementation(((str: string) => str) as any);
        }
    });

    it('expandToRawHtml (real implementation) honors expandMath: false', async () => {
        const actual = await vi.importActual<typeof import('../../../src/utils/noteCitationExpand')>(
            '../../../src/utils/noteCitationExpand'
        );
        const meta = { elements: new Map() } as any;
        const s = 'the value of $x+y$ in the model';
        expect(actual.expandToRawHtml(s, meta, 'old')).toContain('class="math"');
        expect(
            actual.expandToRawHtml(s, meta, 'old', undefined, undefined, undefined, { expandMath: false })
        ).toBe(s);
    });

    it('expandToRawHtml (real implementation) preserves only ranged dollars', async () => {
        const actual = await vi.importActual<typeof import('../../../src/utils/noteCitationExpand')>(
            '../../../src/utils/noteCitationExpand'
        );
        const meta = { elements: new Map() } as any;
        const copiedAnchor = 'JSON keys "$schema" and "$ref". ';
        const replacement = `${copiedAnchor}Formula: $x$`;
        expect(actual.expandToRawHtml(
            replacement,
            meta,
            'new',
            undefined,
            undefined,
            undefined,
            { literalDollarRanges: [{ start: 0, end: copiedAnchor.length }] },
        )).toBe(`${copiedAnchor}Formula: <span class="math">$x$</span>`);
    });

    it('expandToRawHtml (real implementation) preserves dollar ordinals', async () => {
        const actual = await vi.importActual<typeof import('../../../src/utils/noteCitationExpand')>(
            '../../../src/utils/noteCitationExpand'
        );
        const meta = { elements: new Map() } as any;
        const source = 'Formula $x$ uses "$schema" and "$ref"';
        expect(actual.expandToRawHtml(
            source,
            meta,
            'old',
            ...
            undefined,
            undefined,
            { literalDollarOrdinals: [2, 3] },
        )).toBe('Formula <span class="math">$x$</span> uses "$schema" and "$ref"');
    });

    it('expandToRawHtml (real implementation) excludes escaped dollars from ordinals', async () => {
        const actual = await vi.importActual<typeof import('../../../src/utils/noteCitationExpand')>(
            '../../../src/utils/noteCitationExpand'
        );
        const meta = { elements: new Map() } as any;
        const source = 'Formula $x+\\$5$ uses "$schema" and "$ref"';
        expect(actual.expandToRawHtml(
            source,
            meta,
            'old',
            ...
            undefined,
            { literalDollarOrdinals: [2, 3] },
        )).toBe('Formula <span class="math">$x+\\$5$</span> uses "$schema" and "$ref"');
    });

    it('expandToRawHtml (real implementation) does not open math at an escaped dollar', async () => {
        const actual = await vi.importActual<typeof import('../../../src/utils/noteCitationExpand')>(
            '../../../src/utils/noteCitationExpand'
        );
        const meta = { elements: new Map() } as any;
        expect(actual.expandToRawHtml(
            ...
            meta,
            'old',
        )).toBe('Cost \\$5;<span class="math">$x$</span>');
    });
});
