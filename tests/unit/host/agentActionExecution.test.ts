...
vi.mock('../../../react/utils/editNoteActions', () => ({
    executeEditNoteVariantAction: vi.fn(),
    undoEditNoteVariantAction: vi.fn(),
...
import { executeEditNoteVariantAction } from '../../../react/utils/editNoteActions';
...
describe('note-edit action execution', () => {
    ...
    it('refuses a rejection while the same action is being applied', async () => {
        ...
        vi.mocked(executeEditNoteVariantAction).mockImplementationOnce(
            () => new Promise((resolve) => {
                finishApply = resolve;
            }) as any,
        );
